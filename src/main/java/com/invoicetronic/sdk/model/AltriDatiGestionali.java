  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AltriDatiGestionali.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in AltriDatiGestionali is not found in the empty JSON string", AltriDatiGestionali.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AltriDatiGestionali.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `AltriDatiGestionali` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("tipo_dato") != null && !jsonObj.get("tipo_dato").isJsonNull()) && !jsonObj.get("tipo_dato").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `tipo_dato` to be a primitive type in the JSON string but got `%s`", jsonObj.get("tipo_dato").toString()));
      }
      if ((jsonObj.get("riferimento_testo") != null && !jsonObj.get("riferimento_testo").isJsonNull()) && !jsonObj.get("riferimento_testo").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `riferimento_testo` to be a primitive type in the JSON string but got `%s`", jsonObj.get("riferimento_testo").toString()));
      }
  }
